...
Commit these changes together into one commit
    ...
    for l in cordova-plugman cordova-cli cordova-js cordova-create cordova-common cordova-serve cordova-fetch; do ( cd $l; v="$(grep '"version"' package.json | cut -d'"' -f4)"; git commit -am "$JIRA Updated version and RELEASENOTES.md for release $v" ); done
